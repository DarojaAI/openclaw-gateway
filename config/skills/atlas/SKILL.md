          ...
          [
            {
              "id": "brew",
              "kind": "brew",
              "formula": "ariga/tap/atlas",
              "bins": ["atlas"],
              "label": "Install Atlas CLI (brew)",
            },
            {
              "id": "shell",
              "kind": "script",
              "label": "Install Atlas CLI (curl)",
              "command": "curl -sSf https://atlasgo.sh | sh",
              "bins": ["atlas"],
            },
          ],
      },
...
**Important:** Using the wrong scope causes errors (`ModifySchema is not allowed`) or silently drops database-level objects from migrations. Match the dev URL scope to the project's target database URL. For PostGIS or pgvector schemas, use `docker://postgis/latest/dev` or `docker://pgvector/pg17/dev`.
